...
int main(int argc, char* argv[]) {
  ...
  cout << "Enter the number of threads: ";  // added for parallelization                                                              
  cin >> nThreads; 

  //set the number of threads passed for parallelization.
  omp_set_num_threads( nThreads ); 

  Timer timer;
  timer.start( );

  #pragma omp parallel firstprivate(x,y,radius,i) shared(count) reduction( +:pi)
  {
    ...
    for ( int quad = 0; quad < 4; quad++ ) {
      ...
      for (i = 0; i < niter; i++ ) {
        ...
        radius =  x * x + y * y;
        //increment count if point is within radius.
        if ( radius <= 1 ) count++;
      }
